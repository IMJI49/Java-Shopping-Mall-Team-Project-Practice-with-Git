					...
					if(id.equals("admin")) {
						...
						while(true) {
							...
							if(menu.equals("1")) {
							    ...
							    System.out.println("\n====== 주문 confirm ======");
							    System.out.print("확정할 주문의 주문번호를 입력하세요: ");
							    String orderId = scanner.nextLine();

							    Order order = (Order) managerService.getOrders().get(orderId);
							    if(order == null) {
							        System.out.println("존재하지 않는 주문번호입니다.");
							    } else if(order.getStatus() != Order.Status.PENDING) {
							        System.out.println("이 주문은 이미 확정되었거나 CONFIRM 불가 상태입니다.");
							    } else {
							        try {
							            managerService.confirmOrder(order.getStatus(), orderId); // 상태 확인 및 변경
							            System.out.println("주문이 CONFIRM(확정) 처리되었습니다.");
							        } catch(Exception e) {
							            System.out.println("주문 확정 실패: " + e.getMessage());
							        }
							    }
							    System.out.println("================================\n");
							    ...
							    System.out.println("\n======= 주문 취소 =======");
							    System.out.print("취소할 주문의 주문번호를 입력하세요: ");
							    String orderId = scanner.nextLine();
							    ...
							    if(order == null) {
							        ...
							        System.out.println("이 주문은 이미 처리 중이거나 취소 불가 상태입니다.");
							    } else {
							        try {
							            managerService.cancelOrder(order.getStatus(), orderId);
						...
						while(true) {
							...
							if(menu.equals("1")) {
								...
								while(true) {
							        ...
							        System.out.println("└────────────────────────────────────┘");
							        System.out.print("메뉴를 선택하세요: _");
							        String cartMenu = scanner.nextLine();

							        if(cartMenu.equals("1")) { // 장바구니 조회
							            System.out.println("\n=========== 장바구니 조회 ============");
							            ...
							            System.out.println("=====================================\n");
							        } else if(cartMenu.equals("2")) { // 상품 추가
							            System.out.println("\n========= 상품 추가 =============");
							            ...
							            System.out.println("================================\n");
							        } else if(cartMenu.equals("3")) { // 수량 변경
							            System.out.println("\n=========== 수량 변경 ===============");
							            ArrayList<CartItem> cartItems = userService.getCarts().get(id);
							            if(cartItems == null || cartItems.isEmpty()) {
							            ...
							            System.out.println("=======================================\n");
							        } else if(cartMenu.equals("4")) { // 상품 삭제
							            System.out.println("\n=========== 상품 삭제 ===============");
							            ...
							            boolean removed = cartItems.removeIf(ci -> ci.getItem().getName().equals(name));
							            if(removed) {
							            ...
							            System.out.println("======================================\n");
							        } else if(cartMenu.equals("5")) { // 장바구니 비우기
							            System.out.println("\n============ 장바구니 비우기 =============");
							            ArrayList<CartItem> cartItems = userService.getCarts().get(id);
							            if(cartItems != null) cartItems.clear();
							            System.out.println("장바구니가 비워졌습니다.");
							            System.out.println("=========================================\n");
							        } else if(cartMenu.equals("0")) {
							            break;
							        } else {
							            System.out.println("잘못된 입력입니다. 다시 입력해주세요.");
							        }
							    }
							} else if (menu.equals("4")) { // 주문하기
							    System.out.println("\n=============== 주문하기 ================");
							    try {
							        Customer customerObj = (Customer) userService.getCustomers().get(id);
							        if(customerObj == null) {
							            System.out.println("고객 정보가 없습니다."); return;
							        }
							        ArrayList<CartItem> cartItems = userService.getCarts().get(id);
							        if (cartItems == null || cartItems.isEmpty()) {
							            System.out.println("장바구니가 비어 있습니다. 주문할 상품을 추가하세요.");
							        } else {
							            // 배송지 재입력 여부 선택사항
							            System.out.print("배송지로 등록된 주소 [" + customerObj.getAddress() + "] 사용하시겠습니까? (Y/N): ");
							            String addressYn = scanner.nextLine();
							            String shippingAddress = customerObj.getAddress();
							            if(addressYn.equalsIgnoreCase("N")) {
							                System.out.print("새 배송지를 입력하세요: ");
							                shippingAddress = scanner.nextLine();
							            }
							            userService.placeOrder(customerObj, shippingAddress);
							            // 주문 완료 후 장바구니 비움
							            cartItems.clear();
							        }
							    } catch(Exception e) {
							    ...
							    System.out.println("==========================================\n");
							} else if (menu.equals("5")) { // 주문내역 확인
							    System.out.println("\n============= 주문내역 ===============");
							    try {
							        // 모든 내역 중 내 아이디와 매칭되는 주문만 필터
							        List<Order> myOrders = new ArrayList<>();
							        for(Object o : userService.getOrders().values()) {
							            Order order = (Order)o;
							        ...
							        System.out.println("주문 내역 조회 실패: " + e.getMessage());
							    }
							    System.out.println("========================================\n");
							} else if (menu.equals("6")) {
